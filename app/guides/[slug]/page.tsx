...
export function generateStaticParams(){ return registry.pages.map((page) => ({ slug: page.slug })); }
...
export default async function Guide({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const page = registry.pages.find((item) => item.slug === slug);
  if (!page) notFound();
