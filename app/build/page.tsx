...
const stepLabels = ['Planning Reality Check', 'Recommendation Studio', 'Venue + Lodging', 'Budget + Tradeoffs', 'Design / Scope', 'Vendor Team', 'Ideas', 'Planner Packet'];
  ...
  return <div className="space-y-10 pb-24">
    <section className="grid gap-6 lg:grid-cols-[1fr_360px]">
      <div>
        ...
        <p className="mt-5 max-w-3xl text-lg leading-8 text-charcoal/70">Start with a Planning Reality Check. Then use Recommendation Studio when you are stumped: venue, flowers, budget, guest experience, design, vendors, food, timeline, risks, or anything messy. The app uses your saved constraints everywhere.</p>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <a href="#step-0" className="rounded-full bg-charcoal px-5 py-3 font-bold text-linen">Start with Planning Reality Check</a>
      ...
      <Card className="sticky top-6 h-fit" data-testid="canonical-plan-summary">
        <p className="text-xs uppercase tracking-[0.3em] text-charcoal/50">Canonical constraint state</p>
    ...
    <section id="step-0" className="grid gap-6 lg:grid-cols-[220px_1fr]">
      ...
      <Card data-testid="constraint-profile">
        ...
        <p className="mt-3 text-charcoal/70">Your answer controls which constraint fields appear. Unknown is allowed; the Recommendation Studio can help discover constraints.</p>
        ...
        {plan.constraintMode && <div className="mt-6 space-y-6" data-testid="constraint-fields">
          ...
          {(plan.constraintMode === 'hard' || plan.constraintMode === 'flexible') && <div className="rounded-3xl bg-ivory p-4" data-testid="hard-constraint-detail">
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              <Field label="Full venue buyout"><select data-testid="full-buyout" value={plan.fullBuyout} onChange={event => patch({ fullBuyout: event.target.value })} className="rounded-2xl border border-charcoal/10 bg-white p-3"><option value="">Unknown</option><option value="required">Required</option><option value="preferred">Preferred</option><option value="not required">Not required</option></select></Field>
              <Field label="Onsite sleeping requirement"><select data-testid="onsite-lodging" value={plan.onsiteLodging} onChange={event => patch({ onsiteLodging: event.target.value })} className="rounded-2xl border border-charcoal/10 bg-white p-3"><option value="">Unknown</option><option value="required">Must sleep guests onsite</option><option value="preferred">Preferred</option><option value="vip only">VIP/family onsite only</option><option value="not required">Nearby hotels are fine</option></select></Field>
              ...
              <Field label="Outside catering"><select data-testid="outside-catering" value={plan.outsideCatering} onChange={event => patch({ outsideCatering: event.target.value })} className="rounded-2xl border border-charcoal/10 bg-white p-3"><option value="">Unknown</option><option value="required">Required</option><option value="preferred">Preferred</option><option value="venue catering ok">Venue catering okay</option></select></Field>
    ...
    <section id="step-2" className="grid gap-6 lg:grid-cols-[220px_1fr]">
      ...
      <Card data-testid="venue-finder">
        ...
        <div data-testid="selected-venue-summary" className="mt-5 rounded-2xl bg-ivory p-4 text-sm font-semibold">Selected venue strategies: {chosenVenues.length ? chosenVenues.map(v => v.name).join(' · ') : 'None selected yet'}</div>
    ...
    <section id="step-3" className="grid gap-6 lg:grid-cols-[220px_1fr]">
      ...
      <Card data-testid="budget-reality">
        ...
        <p data-testid="budget-guest-sync" className="mt-5 rounded-2xl bg-white p-4 text-sm font-bold">Guest count synced from Step 0: {plan.guestCount || 'Unknown'}</p>
