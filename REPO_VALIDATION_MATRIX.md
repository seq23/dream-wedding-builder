...
| Content pattern contract | npm run validate:content-pattern | HARD FAIL | every hub, guide and product page carries a direct answer, an h1 that carries the query, a table with no empty cells, and one paid-product conversion path; reports coverage for checklist, comparison table, concrete numbers, named sources, FAQ and JSON-LD | that the answer is correct, or that a reported gap has been closed |
| Internal link graph | npm run validate:internal-links (needs a build; CI runs it after `npm run build`) | HARD FAIL | every URL in every host's real sitemap responds 200 and is the target of at least one same-host anchor in the HTML the built server actually returned - an orphan page in the sitemap can neither receive nor pass authority | that the inbound link is prominent, or that the page deserves to be indexed |
